...
/**
 * Creates a file card with a 4-column optimized layout.
 */
function createFileCard(fileDoc, userLookup, currentUserId) {
    ...
    return `
        <div class="col">
            <div class="card dashboard-card file-card h-100 transition-all" data-doc-id="${fileDoc.$id}">
...
function getFilesHTML(fileList) {
    // Dynamic Class: Grid if files exist, Centered Flexbox if empty
    const containerClass = fileList.length > 0
        ? "row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4"
        : "d-flex flex-column align-items-center justify-content-center text-center py-5";

    const contentHTML = fileList.length > 0
        ? '' // Content will be filled by mapping, handled in render if initially not empty
        : `
    ...
    <div class="files-view-container container-fluid py-4 px-md-5">
        ...
        <div id="file-cards-container" class="${containerClass}" style="min-height: 300px;">
            ${contentHTML}
        </div>
...
function attachEventListeners(currentUser, userLookup) {
    ...
    // HELPER: Toggle classes based on content
    ...
    const renderGrid = (files) => {
        ...
        grid.innerHTML = files.map(doc => createFileCard(doc, userLookup, currentUser.$id)).join('');
    };
